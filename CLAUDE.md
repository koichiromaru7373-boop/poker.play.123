...
- `compare_v5_v6.py` — v5とv6の直接対戦(v5には先頭68次元だけ見せるObsSliceアダプタ)。v6の真の判定はこれで行う(vs GTO-Rule値はNFSPに不利な指標のため)
- v6試運転(30k)は vs GTO-Rule ±1で推移 — NFSPは立ち上がりが遅いため正常範囲。判断は数十万ハンド後
- play.html スマホ対応済み(640px以下でレスポンシブ)。スマホから遊ぶには `uvicorn api_server:app --host 0.0.0.0` で起動し、同じWi-FiからPCのIPで `http://<PCのIP>:8000/play`
- クラウドデプロイ対応済み: `export_model.py`(リプレイメモリ除去の軽量モデルを `models/` に出力、Git同梱可)、`requirements-server.txt`(CPU版torch)、`render.yaml`(Render Blueprint)、環境変数 `ACCESS_TOKEN` による簡易ガード(未設定ならチェックなし)。MODEL_PATHSは models/ を最優先で読む
- 残TODO(優先順): ①/analysis(即時解説) ②v6モデルのAPI載せ替え(extended_env_v6) ③6人卓のv5/v6化 ④永続化・本格認証
